...
// Use jemalloc, compiled with unique prefix (--with-jemalloc-prefix= configure
// option)
...
protected:
    ...
    // If random number from range (0, MaxBucketSize) is lower than
    // m_bucketSize, an operation will be performed
...
public:
    ...
    Operation(OperationName name, unsigned bucketSize = MaxBucketSize)
        : m_name(name), m_bucketSize(bucketSize)
    {
    ...
    // Check if operation should be performed (currently drawn random number
    // lower than bucket size)
    ...
    virtual void perform(const memkind_t &kind, void *&mem, size_t size = 0,
                         size_t offset = 0, size_t alignment = 0) const = 0;
};
...
{
public:
    MallocOperation(OperationName name) : Operation(name)
    {}

    MallocOperation(OperationName name, unsigned bucketSize)
        : Operation(name, bucketSize)
    {}

    virtual void perform(const memkind_t &kind, void *&mem, size_t size,
                         size_t offset, size_t alignment) const override
    {
        EMIT(2,
             "Entering Operation::" << getNameStr() << ", size=" << size
        ...
        EMIT(2,
             "Exiting Operation::" << getNameStr() << ", size=" << size
                                   << ", offset=" << offset << ", alignment="
                                   << alignment << ", mem=" << mem)
    }
};
...
{
public:
    JemallocOperation(OperationName name) : Operation(name)
    {}

    JemallocOperation(OperationName name, unsigned bucketSize)
        : Operation(name, bucketSize)
    {}
...
{
public:
    JemkmallocOperation(OperationName name) : Operation(name)
    {}

    JemkmallocOperation(OperationName name, unsigned bucketSize)
        : Operation(name, bucketSize)
    {}
...
public:
    ...
    MemkindOperation(OperationName name) : Operation(name)
    {}

    MemkindOperation(OperationName name, size_t bucketSize)
        : Operation(name, bucketSize)
    {}
                         ...
                         size_t offset, size_t alignment) const override
    {
        ...
        switch (m_name) {
            ...
            case Align:
            {
                ...
                // randomly choose alignment from (sizeof(void*), sizeof(void*)
                // * MemalignMaxMultiplie)
